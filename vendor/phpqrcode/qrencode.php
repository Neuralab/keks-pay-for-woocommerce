    ...
    */
    ...
    class QRcode {
        ...
        */
            ...
            if ($elemId === false) {
                ...
                if ($width == false) {
                    if (($size !== false) && ($size > 0))  {
						...
						viewBox="'.(-$margin).' '.(-$margin).' '.($area->getWidth()+($margin*2)).' '.($area->getWidth()+($margin*2)).'" width="' . $width . '" height="' . $width . '" style="enable-background:new 0 0 550 550;" xml:space="preserve">';
        ...
        public static function svg_design($text, $elemId = false, $outFile = false, $level = QR_ECLEVEL_L, $width = false, $size = false, $margin = 4, $compress = false, $lang = 0, $design_template = 0)
        {
            $enc = QRencode::factory($level, 1, 0);
            $tab_src = $enc->encode($text, false);
            $area = new QRsvgOutput($tab_src);
            ...
            $design = '';
            require plugin_dir_path( __FILE__ ).'../lib/t'.$lang.$design_template.'.php';
	 ...
	 width="550px" height="550px" viewBox="0 0 550 550" style="enable-background:new 0 0 550 550;" xml:space="preserve">'."\n";
						...
						$svg .= $design;
            $svg .= '<rect xmlns="http://www.w3.org/2000/svg" height="216" width="216" x="167" y="142" fill="white" /><g transform="translate(175,150) scale(8)">' . $area->getRawSvg() . '</g></svg>';
